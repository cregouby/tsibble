...
dat_x <- tibble(
  start = idx_day,
  stop = idx_day + ddays(4),
  value = rnorm(5)
)
nyc_bikes <- build_tsibble(tsibbledata::nyc_bikes, key = bike_id, index=start_time, index2=stop_time, ordered = TRUE , interval=FALSE)
...
test_that("an irregular tbl_ts", {
  tsbl <- build_tsibble(dat_x, index = start, index2=stop, interval = FALSE)
  expect_identical(fill_gaps(tsbl), tsbl)
  expect_identical(has_gaps(tsbl), tibble(.gaps = FALSE))
})

test_that("a tbl_ts without implicit missing values", {
  tsbl <- build_tsibble(dat_x, index = start, index2=stop)
  expect_identical(fill_gaps(tsbl), tsbl)
  ref_tbl <- tibble(
...
dat_y <- dat_x[c(1:3, 5), ]
tsbl <- build_tsibble(dat_y, index = start, index2=stop, interval = FALSE)

test_that("a tbl_ts of 1 day interval with no replacement", {
